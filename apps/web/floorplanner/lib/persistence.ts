...
import { getFloorplannerStorageNamespace } from './runtime'
...
function getStorageKeys() {
  const namespace = getFloorplannerStorageNamespace().trim() || 'default'
  const prefix = `floorplanner:${namespace}`
  return {
    storageKey: `${prefix}:layout`,
    manifestKey: `${prefix}:manifest`,
    layoutPrefix: `${prefix}:layouts:`,
  }
}
...
export function saveToLocalStorage(slice: DocumentSlice): SaveError | null {
  try {
    const { storageKey, layoutPrefix } = getStorageKeys()
...
function loadManifest(): LayoutManifest {
  try {
    const { manifestKey } = getStorageKeys()
...
export function recoverLayoutsFromStorage(): number {
  ...
  for (let i = 0; i < localStorage.length; i += 1) {
    ...
    if (!key || !key.startsWith(layoutPrefix)) continue

    const id = key.slice(layoutPrefix.length)
  ...
  if (activeLayoutId) {
    const raw = localStorage.getItem(layoutPrefix + activeLayoutId)
    if (raw) {
      try {
        const payload = migrate(JSON.parse(raw) as PersistedPayload)
        payload.data = validateDocumentSlice(payload.data)
...
export function saveLayoutAs(name: string, slice: DocumentSlice): string {
  ...
  localStorage.setItem(layoutPrefix + id, JSON.stringify(payload))
...
export function clearAllLayouts(): void {
  const { layoutPrefix, manifestKey, storageKey } = getStorageKeys()
  const manifest = loadManifest()
  for (const layout of manifest.layouts) {
    localStorage.removeItem(layoutPrefix + layout.id)
  }
...
export function deleteLayout(id: string): void {
  ...
  localStorage.removeItem(layoutPrefix + id)
  const manifest = loadManifest()
