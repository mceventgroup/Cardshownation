...
export function CookieConsent({ initialConsent }: { initialConsent: Consent | null }) {
  ...
  return <div className="fixed inset-x-4 bottom-4 z-[100] mx-auto max-w-2xl rounded-3xl border border-slate-200 bg-white p-5 shadow-2xl" role="dialog" aria-label="Cookie choices">
    ...
    <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:justify-end">
      <button type="button" disabled={!hydrated} onClick={() => choose("essential")} className="rounded-full border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 disabled:cursor-wait disabled:opacity-60">Essential only</button>
      <button type="button" disabled={!hydrated} onClick={() => choose("optional")} className="rounded-full bg-brand-600 px-4 py-2 text-sm font-semibold text-white disabled:cursor-wait disabled:opacity-60">Accept optional</button>
    </div>
  </div>;
}
