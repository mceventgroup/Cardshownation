...
export function CookieConsent({ initialConsent, globalPrivacyControl }: { initialConsent: Consent | null; globalPrivacyControl: boolean }) {
  ...
  return <div id="cookie-preferences-dialog" ref={dialogRef} tabIndex={-1} onKeyDown={(event) => { if (event.key === "Escape") closePanel(); }} className="fixed inset-x-2 bottom-2 z-[100] mx-auto max-w-xl rounded-2xl border border-slate-200 bg-white p-4 shadow-2xl sm:inset-x-4 sm:bottom-4 sm:p-5" role="dialog" aria-labelledby="cookie-dialog-title" aria-describedby="cookie-dialog-description">
    ...
    <p id="cookie-dialog-description" className="mt-1.5 text-xs leading-5 text-slate-600 sm:text-sm sm:leading-6">We use essential cookies. Analytics and ads stay off unless you allow them. <Link href="/cookies" className="font-semibold text-brand-700 underline-offset-4 hover:underline">Cookie Policy</Link>.</p>
    ...
    <div className="mt-3 grid grid-cols-2 gap-2 sm:flex sm:justify-end">
      <button type="button" disabled={!hydrated} onClick={() => choose("essential")} className="min-h-11 rounded-xl border border-slate-300 px-3 py-2 text-xs font-semibold text-slate-700 disabled:cursor-wait disabled:opacity-60 sm:px-4 sm:text-sm">Essential only</button>
      <button type="button" disabled={!hydrated || globalPrivacyControl} onClick={() => choose("optional")} className="min-h-11 rounded-xl border border-slate-300 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60 sm:px-4 sm:text-sm">Allow optional cookies</button>
    </div>
  </div>;
}
