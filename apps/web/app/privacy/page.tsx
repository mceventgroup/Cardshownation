  ...
  return (
    <LegalPage title="Privacy Policy" summary="This policy explains what information Card Show Nation handles, why we use it, and the choices available to you." updated="September 15, 2026" currentPath="/privacy" sections={sections}>
      ...
      <LegalSection id="sharing" title="5. How information is shared">
        ...
        <p>The homepage sends your current public IP address from our server to GeoJS to estimate your state, with our hosting provider&apos;s estimate as a fallback when it describes the visitor rather than a proxy. A saved account or browser state is used only when automatic detection is unavailable. We do not send account information or device GPS coordinates to GeoJS. State lookup results may be held in server memory for up to 15 minutes under a hashed IP identifier; they are not saved to your account or a location cookie. Choosing a state adds a filter to the page address and skips this lookup for that page. Select &quot;Use my IP location&quot; to return to automatic detection. See <a href="https://www.geojs.io/privacy/" className="font-semibold text-brand-700 underline-offset-4 hover:underline">GeoJS&apos;s privacy policy</a>.</p>
