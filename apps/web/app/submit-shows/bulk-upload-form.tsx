...
export function PublicBulkUploadForm() {
  ...
  async function readFile(file: File) {
    ...
    if (file.size > MAX_PUBLIC_BULK_UPLOAD_BYTES) {
      setParseError("That file is too large. Keep the CSV under 750 KB.");
      return;
    }
    ...
    const missing = getMissingBulkHeaders(parsed.meta.fields ?? []);
    if (missing.length > 0) {
    ...
    <form action={action} className="mt-8 space-y-6" data-analytics-form="submit-shows-bulk">
      <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">
              Use one row per show date. You can fill it out in Excel, Numbers, or Google Sheets, then save it as a CSV.
            </p>
          </div>
          <a href="/show-upload-template.csv" download className="inline-flex min-h-11 shrink-0 items-center justify-center gap-2 rounded-full border border-brand-200 bg-brand-50 px-5 py-2.5 text-sm font-semibold text-brand-800 transition-colors hover:bg-brand-100">
        ...
        <div className="mt-5 grid gap-3 text-sm sm:grid-cols-2">
          ...
          <div className="rounded-2xl bg-slate-50 p-4"><strong className="text-slate-800">Helpful extras:</strong><p className="mt-1 text-slate-600">Hours, address, admission, categories, and links</p></div>
        </div>
      </section>

      <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
        <p className="text-sm font-semibold text-brand-700">Step 2</p>
        <h2 className="mt-1 text-xl font-semibold text-slate-950">Choose your completed CSV</h2>
        <label className="mt-5 flex cursor-pointer flex-col items-center justify-center rounded-3xl border-2 border-dashed border-slate-200 bg-slate-50 px-6 py-10 text-center transition-colors hover:border-brand-300 hover:bg-brand-50">
          ...
          <span className="mt-3 font-semibold text-slate-800">{fileName || "Choose a CSV file"}</span>
          <span className="mt-1 text-sm text-slate-500">Up to 100 shows in one upload</span>
          <input ref={fileRef} name="file" type="file" accept=".csv,text/csv" required className="sr-only" onChange={(event) => { const file = event.target.files?.[0]; if (file) void readFile(file); }} />
        </label>
